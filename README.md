...
Боевой домен — `vent-clean.ru` (reg.ru, DNS хостинга reg.ru: `ns1.hosting.reg.ru`).
В зоне стоят A-записи `@` и `www` на `76.76.21.21`; апекс — основной адрес, `www` и
`vent-final.vercel.app` уходят на него 301-м. Канонический адрес по умолчанию задан в
`src/lib/site.ts`; переменная `NEXT_PUBLIC_SITE_URL` в Production переопределяет его и
нужна только на время переездов.
