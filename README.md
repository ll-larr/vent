# Vent Clean — промышленная чистка вентиляции
...
| `NEXT_PUBLIC_SITE_URL` | Канонический URL сайта (метаданные, sitemap, JSON-LD). По умолчанию `https://vent-clean.ru` |
...
|---|---|---|
| `vent.team` | `76.76.21.21` (апекс Vercel) | `404 DEPLOYMENT_NOT_FOUND` |
| `vent-clean.ru` | нет | не резолвится |

A-запись Vercel перенесена с `vent-clean.ru` на `vent.team`, но сам домен в проекте
`vent-final` не добавлен — Vercel получает запрос на хостнейм, которого не знает, и отдаёт
404. Лечится одним из двух: добавить `vent.team` в домены проекта на Vercel, либо сменить
хостинг и переписать A-запись на новый IP. MX ни на одном домене нет — почта
`hello@vent.team` и `privacy@vent.team` из `src/lib/site.ts` и страницы политики пока не
работает.
